(()=>{
  ...
  async function publish(){const token=localStorage.getItem(TOKEN_KEY)||'';if(!token)throw Error('missing token');const raw=localStorage.getItem(STORE_KEY);if(!raw)throw Error('missing content');const r=await fetch('/api/content',{method:'POST',headers:{'content-type':'application/json; charset=utf-8','x-nascw-admin-token':token},body:raw});const j=await r.json().catch(()=>({}));if(!r.ok)throw Error(j.error||'فشل النشر')}
  function write(partners,doPublish){const c=read();c.partners=normalize(partners);c.layout=c.layout||{};if(!Array.isArray(c.layout.order))c.layout.order=['hero','pain','ba','services','offers','proof','team','process','why','start','faq'];c.layout.order=c.layout.order.filter(x=>x!=='partners');const i=c.layout.order.indexOf('faq');c.layout.order.splice(i>=0?i:c.layout.order.length,0,'partners');localStorage.setItem(STORE_KEY,JSON.stringify(c));if(doPublish)publish().then(()=>toast('تم حفظ ونشر صورة الشريك ✓')).catch(e=>toast(e.message,true));return c}
  ...
  function showEditor(){
    ...
    const c=read();let p=normalize(c.partners);write(p,false);
    ...
    const save=()=>{write(p,false);try{window.dispatchEvent(new Event('input'))}catch(e){}};
    ...
    function drawList(){
      ...
      p.items.forEach((m,i)=>{
        ...
        input.onchange=async e=>{const f=e.target.files&&e.target.files[0];if(!f)return;try{toast('جاري رفع الصورة...');m.photo=await uploadImage(f);write(p,true);drawPhoto()}catch(err){toast(err.message,true)}};
  ...
  function inject(){const side=$('#side');if(!side)return;let a=$('#partners-admin-entry');if(!a){a=ce('a',{id:'partners-admin-entry',innerHTML:icon+'<span>الشركاء الاستراتيجيون</span>'});a.onclick=showEditor;const team=[...side.querySelectorAll('a')].find(x=>/فريق العمل/.test(x.textContent));if(team)team.after(a);else side.appendChild(a)}}
  function boot(){inject();const side=$('#side');if(side&&!side.__partnersForce){side.__partnersForce=1;new MutationObserver(inject).observe(side,{childList:true,subtree:true})}const b=$('#btn-save');if(b&&!b.__partnersForce){b.__partnersForce=1;b.addEventListener('click',()=>write(normalize(read().partners),false),true)}}
